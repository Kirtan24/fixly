...
exports.getListings = asyncHandler(async (req, res) => {
  ...
  // Check if request is from admin (has admin role or specific admin query param)
  const isAdminRequest = req.user?.userType === 'admin' || req.query.admin === 'true';
  ...
  // Filter by status if provided (admin feature)
  if (req.query.status) {
